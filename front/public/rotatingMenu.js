  ...
  $.fn.extend({
    rotatingMenu: function (config) {
      ...
      var topPos = menuHeight / 2 - divHeight / 2,
        topPos_distance = tdiv_odd
          ? Math.floor(topPos / dHalf)
          : Math.floor(topPos / (dHalf - 1)),
        lPos = 0,
        lPos_distance = Math.floor(
          (menuWidth - divWidth * (tdivhaf + 1)) / tdivhaf + divWidth
        );
      ...
      var tdiv_bottom = tdiv - tdivhaf,
        ...
        tdiv_bottom_pos = Math.floor(
          (menuWidth - divWidth * (tdiv_bottom + 1)) / tdiv_bottom + divWidth
        ),
        bPos_distance = bdiv_odd
          ? Math.floor(bottomPos / bdiv_half)
          : Math.floor(bottomPos / (bdiv_half - 1));
      var count = 0,
      ...
      var rm = {
        ...
        menuPosition: function () {
          for (i = 1; i <= tdiv; i++) {
            if (i <= tdivhaf) {
              $(rEvent)
                .children(config.divId + i)
                .css({ top: topPos, left: lPos });
        ...
        menuReset: function (cont) {
          ...
          if (count == tdiv || (typeof cont != "undefined" && cont)) {
            ...
            for (i = 0; i < tdiv; i++) {
              if (
                leftAnim &&
                ((rpos[i].top > zPos && rpos[i].left > 0) ||
                  rpos[i].left >= leftLimit)
              ) {
                zHigh--;
        ...
        rmAnimate: function () {
          for (i = 1; i <= tdiv; i++) {
            $(rEvent)
              .children(config.divId + i)
              .animate(
                { top: mapPos["menu" + i].top, left: mapPos["menu" + i].left },
                speed,
                "linear",
                function () {
                  rm.menuReset();
