...
const Index = () => {
  ...
  }, [st_faqDeleteDone, router.query]);

  useEffect(() => {
    if (st_faqListError) {
      return message.error(st_faqListError);
    }
  }, [st_faqListError]);
        ...
        <Row gutter={[10, 10]} style={{ padding: "0 0 10px 0" }}>
          ...
          <Col>
            <Button
              onClick={() =>
                moveLinkHandler(
                  `/admin/faq?page=${currentPage}&search=${inputSearch.value}`
                )
              }
            >
              <SearchOutlined />
